...
<script>
    ...
    export default {
        ...
        methods: {
            //异步获取验证码
            getCode() {
                if (!this.computeTime) { //只有当第一次执行 或者 this.computeTime减到0时才执行倒计时功能（防止多次点击出现bug）
                    this.computeTime = 10;
            ...
            //每次获取不同的验证码
            getCaptcha() {
                this.$refs.captcha.src = 'http://localhost:4000/captcha?time=' + Date.now(); //如果没有？time Date.now(),则每次请求的src不会改变，导致验证码不会变
            }
        }
    }
</script>
